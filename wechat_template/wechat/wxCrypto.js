...
class WxCrypt {
    constructor(opts){
        ...
        this.decrypt(this.msg_signature)
    }
    encrypt (xmlMsg) {
        console.log('执行1')
        /*
    ...
    decrypt(text) {
        ...
        console.log('执行一次吧')
        var buffer1 = decipher.update(text, 'base64');
        // var buffer2 = decipher.final('utf-8');
        return
        console.log('执行2',buffer1,buffer2)
        return
        let decipheredBuff = Buffer.concat([buffer1,buffer2])
